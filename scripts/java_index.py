                ...
                if os.path.exists(loadpath):
                    for file in os.listdir(loadpath):
                        ...
                        with open(os.path.join(loadpath, file), 'r', encoding='utf-8') as f:
                            file_list = f.readlines()
                        java_list = [line.strip() for line in file_list]
                        save_list = os.listdir(os.path.join(datapath, group, project, version, 'recommended_IRBL', 'combined_files', bug_id))
                        for java in java_list:
                            if java not in save_list:
                                print(f'WARNING: {group}/{project}/{version} Bug{bug_id}:{java} not found!')
                        if len(java_list) != len(save_list):
                            print(f'WARNING: data count: {len(java_list)}, copy count: {len(save_list)}')
